...
import { utils, Provider, Signer, Contract, Wallet } from 'zksync-ethers';
...
export default function Home() {
  ...
  const sendSponsoredTransaction = async () => {
    ...
    const abi = ['function mint(address to, uint256 quantity) public'];
    const paymasterAddress = '0x5407B5040dec3D339A9247f3654E59EEccbb6391';

    const zkProvider = new Provider(provider.connection.url);
    const signer = provider.getSigner();

    // const zkSigner = Signer.from(signer, chainId, zkProvider);

    // (cc): signer should be zkSigner but it has a type error
    ...
    try {
      setIsPending(true);
      // txn end up being a message to sign instead of a transaction
                  ...
                  <div className='flex flex-col sm:flex-row gap-2 w-full'>
                    ...
                    <button
                      className={`rounded-full border border-solid transition-colors flex items-center justify-center text-white gap-2 text-sm h-10 px-5 font-[family-name:var(--font-roobert)] w-full sm:flex-1
                        ${
                          isPending || transactionReceipt?.status === 'success'
